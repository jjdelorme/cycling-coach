...
## [v1.14.1-beta] - 2026-04-25

Beta release adding the **data-migrations framework (Campaign 23)** and a related ICU stream-parser bug fix surfaced while validating the framework's first migration.

### Features — Campaign 23: Data Migrations Framework
- **feat(migrations): Python data-migration runner parallel to SQL migrations** — new `server/data_migrate.py` discovers `data_migrations/NNNN_*.py` modules, invokes their `run(conn) -> dict` functions in filename order, and persists each result as JSONB in a new `data_migrations` tracking table (separate from `schema_migrations`). Already-applied modules are skipped on re-runs. Runs after `python -m server.migrate` in the merged Cloud Build `migrate` step (one Cloud SQL Proxy, one container pull); also called from the FastAPI lifespan startup hook for local dev (non-fatal on failure so dev boot stays resilient). Step timeout raised to 1200s in `cloudbuild.yaml` + `cloudbuild-test.yaml` for ICU-rate-limited backfills.
- **feat(migrations): 0001_backfill_ride_start_geo** — first data migration. Walks every ride with `start_lat IS NULL` whose filename matches `icu_*`, re-fetches streams from intervals.icu, and lets the (now-fixed) `_backfill_start_location` populate the row. Honors `INTERVALS_ICU_DISABLED` for local dev without ICU credentials (records as `{"skipped": true, "reason": "icu_disabled"}` so the migration still marks applied). Auto-applies on deploy — supersedes the manual `scripts/backfill_ride_start_geo.py` operator step that the Campaign 17 follow-up was waiting on.
- **docs(AGENTS.md): Data Migrations section** — covers how the runner works, when to add a data migration vs. an operator script, conventions (`NNNN_snake_case.py`, idempotent `run(conn)`, ICU-disabled honoring), and the explicit "DB-only schema in `migrations/`, Python-only data ops in `data_migrations/`" boundary.

### Fixes
- **fix(sync): handle ICU typed-entry latlng shape; defense-in-depth write-site guard** — the ICU `/api/v1/activity/{id}/streams` endpoint returns streams as a list of typed entries with parallel `data` (lats) and `data2` (lons) arrays: `[{"type": "latlng", "data": [...lats], "data2": [...lons]}, ...]`. `_extract_streams` previously only read `s.get("data")` and silently dropped `data2`. Downstream `_normalize_latlng` then saw a flat list of bare latitudes; its "concatenated" detection fails when leading GPS samples are `None` (no fix yet), and the flat-alternating fallback paired consecutive latitudes as `(lat, lon)` — the same Syria-bug signature the v1.12.5-beta hotfix was meant to eradicate. Verified live against the `svc-pgdb` shared dev DB: ride 2814 (`icu_i134594382`) real location `(36.59, -105.45)` Taos NM, broken parser produced `(36.59, 36.59)` Syria. Two-layer fix: (1) `_extract_streams` now zips `data` + `data2` for `latlng` typed entries upstream so `_normalize_latlng` hits its existing nested-pair branch; (2) `_backfill_start_location` got an `ABS(lat-lon) >= 1.0` write-site guard that refuses to write any pair matching the corruption signature, plus a new `overwrite_corrupt: bool = False` flag for use by future repair migrations. The guard is intentionally conservative — rare false negatives near the equator at ~0°E (Gulf of Guinea) get "GPS not backfilled" instead of "coordinates reverse-geocode to Syria."
- *Empirical 0002 decision (no migration shipped):* queried `svc-pgdb` for the (lat, lat) corruption signature (`SELECT count(*) FROM rides WHERE start_lat IS NOT NULL AND ABS(start_lat - start_lon) < 1.0`) — returned **zero rows**. No historical-cleanup migration is needed; the forward fix prevents new corruption and there's nothing existing to repair.

### Tests
- **427 unit tests pass** (was 422 in v1.14.0) — added 8 unit tests in `test_sync_latlng.py` covering: `_extract_streams` typed-entry shape with and without `data2`; non-`latlng` types unchanged; sanitized real ICU response from ride 2814 saved at `tests/fixtures/icu_streams_typed_entries.json` (locks down the production API shape and asserts the full chain writes Taos coords, not Syria); explicit `(lat, lat)` write-site rejection; `overwrite_corrupt` SQL predicate change.
- **235 integration tests pass on local `coach_test`** (was 230 in v1.14.0) — added 7 new integration tests for the data-migrations framework (4 for the runner: applies-and-records, idempotent re-run, JSONB persistence, failed-migration-not-recorded; 3 for the 0001 migration: only-null-icu-rides-with-gps, idempotent, ICU-disabled short-circuit) plus 1 end-to-end integration test (`test_migration_handles_real_icu_typed_entry_shape`) that runs the migration against the test DB with `fetch_activity_streams` monkeypatched to return the real ride-2814 fixture and asserts the row lands in the Taos coordinate range — the test that would have caught the `_extract_streams` bug end-to-end.
- **Live svc-pgdb dry-run validated** — ran the restored 0001 against `svc-pgdb` ride 2814 with real ICU credentials; backfilled `(start_lat=36.59369, start_lon=-105.449745)` matching real Taos NM coordinates. `data_migrations` tracking table created in test, no historical pollution.

### Notes
- Schema change: new `data_migrations` tracking table is created on first run by `server/data_migrate.py` itself (not a SQL migration). Idempotent `CREATE TABLE IF NOT EXISTS`.
- No new env vars; existing `INTERVALS_ICU_API_KEY` / `INTERVALS_ICU_ATHLETE_ID` / `INTERVALS_ICU_DISABLED` already cover the migration's needs.
- *Coordination with Campaign 20 (planned, in flight on `feat/ride-map`):* C23's `_extract_streams` fix likely subsumes part of C20 Phase 7 (planned `_normalize_latlng` lat-only-variant detection). C20 Phase 7 should re-audit what's still broken after this beta deploys before implementing. Roadmap entry updated with the coordination note.
- *Coordination with Campaign 24 (was tracking 10 integration test failures):* re-running the integration suite post-merge of `origin/main` shows all 10 failures fixed by main's recent test-hardening commits (`25ecde7` + others). Campaign 24 entry removed from roadmap as obsolete.
