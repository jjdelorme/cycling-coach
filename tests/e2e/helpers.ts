 ...
 * at 1440×900 where the top nav is always visible. As of the React Router
 * migration, header tabs are anchor (`<a>`) elements with role `link`.
 */
import { type Page, expect } from '@playwright/test'

export const BASE_URL = process.env.BASE_URL ?? 'http://localhost:8080'

/** Click a desktop-header nav link by its visible label text. */
export async function navTo(page: Page, label: 'Dashboard' | 'Rides' | 'Calendar' | 'Analysis' | 'Nutrition') {
...
export async function navToSettings(page: Page) {
  await page.locator('header a[title="Settings"], header button[title="Settings"]').first().click()
}
